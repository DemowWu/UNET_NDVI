...
# https://github.com/ForrestPi/VAEGAN/tree/master/src/loss
...
class SSIMLoss(nn.Module):
    def __init__(self, window="gaussian", method="lcs", padding=0, val_range=1, window_size=11, sigma=None, size_average=False):
        ...
        self.ssim = partial(rec_ssim, window_size=window_size, padding=padding, val_range=val_range, method=method)
        if window == "gaussian":
            win = gaussian(window_size, sigma)
            win = create_window(win, channel=1)
            self.ssim = partial(win_ssim, window=win, padding=padding, val_range=val_range, method=method)
